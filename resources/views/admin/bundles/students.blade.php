                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="sort" data-plugin-selectTwo class="form-control populate">
                                ...
                                <option value="rate_asc" @if (request()->get('sort') == 'rate_asc') selected @endif>
                                    {{ trans('update.rate_ascending') }}</option>
                                <option value="rate_desc" @if (request()->get('sort') == 'rate_desc') selected @endif>
                                    {{ trans('update.rate_descending') }}</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="group_id" data-plugin-selectTwo class="form-control populate">
                                ...
                                @foreach ($userGroups as $userGroup)
                                    <option value="{{ $userGroup->id }}" @if (request()->get('group_id') == $userGroup->id) selected @endif>
                        ...
                        <div class="form-group">
                            ...
                            <select name="role_id" class="form-control">
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" @if ($role->id == request()->get('role_id')) selected @endif>
                                        {{ $role->caption }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="status" data-plugin-selectTwo class="form-control populate">
                                ...
                                <option value="expire" @if (request()->get('status') == 'expire') selected @endif>
                                    {{ trans('panel.expired') }}</option>
                            </select>
                        </div>
                    </div>
    ...
    <div class="card">
        <div class="card-header">
            @can('admin_webinar_notification_to_students')
                <a href="{{ getAdminPanelUrl() }}/bundles/{{ $bundle->id }}/sendNotification"
                    class="btn btn-primary mr-2">{{ trans('notification.send_notification') }}</a>
            @endcan

            @can('admin_enrollment_add_student_to_items')
                <button type="button" id="addStudentToCourse"
                    class="btn btn-primary mr-2">{{ trans('update.add_student_to_bundle') }}</button>
            @endcan
                        ...
                        <tr>
                            ...
                            <td class="text-left">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="media-body ml-1">
                                        ...
                                        @if ($student->mobile)
                                            <div class="text-primary text-small font-600-bold">{{ $student->mobile }}
                                            </div>
                                        @endif
                            ...
                            <td>
                                @if (empty($student->id))
                                    ...
                                    <div class="mt-0 mb-1 font-weight-bold text-warning">
                                        {{ trans('update.unregistered') }}</div>
                                @elseif(!empty($bundle->access_days) and !$bundle->checkHasExpiredAccessDays($student->purchase_date, $student->gift_id))
                                    <div class="mt-0 mb-1 font-weight-bold text-warning">{{ trans('panel.expired') }}
                                    </div>
                                @else
                                    <div class="mt-0 mb-1 font-weight-bold text-success">{{ trans('admin/main.active') }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="text-center mb-2" width="120">
                                @if (!empty($student->id))
                                    ...
                                    @can('admin_users_impersonate')
                                        <a href="{{ getAdminPanelUrl() }}/users/{{ $student->id }}/impersonate"
                                    ...
                                    @can('admin_users_edit')
                                        ...
                                        <a href="{{ getAdminPanelUrl() }}/bundles/registered-webinars/{{ encrypt($student->id) }}/{{ $bundle->id }}"
                                            class="btn-transparent  text-info" data-toggle="tooltip" data-placement="top"
                                            title="{{ trans('admin/main.edit') }}">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                    @endcan
        ...
        <div class="mt-25">
            <form action="{{ getAdminPanelUrl() }}/enrollments/store" method="post">
                ...
                <div class="form-group">
                    ...
                    <select name="user_id" class="form-control user-search"
                ...
                <div class="d-flex align-items-center justify-content-end mt-3">
                    <button type="button"
                        class="js-save-manual-add btn btn-sm btn-primary">{{ trans('public.save') }}</button>
                    <button type="button"
