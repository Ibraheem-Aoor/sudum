...
class HomeController extends Controller
{
    public function index()
    {
        ...
        if (in_array(HomeSection::$latest_classes, $selectedSectionsName)) {
            $latestWebinars = Webinar::where('status', Webinar::$active)->where('private', false)->where('category_id', 528)
                ->orderBy('updated_at', 'desc')
                ->with([
                    'teacher' => function ($qu) {
                        $qu->select('id', 'full_name', 'avatar');
        ...
        if (in_array(HomeSection::$best_rates, $selectedSectionsName)) {
            ...
            */
            ...
            $bestRateWebinars = Webinar::where('status', Webinar::$active)->where('private', false)->where('category_id', 526)
                ->orderBy('updated_at', 'desc')
                ->with([
                    'teacher' => function ($qu) {
                        $qu->select('id', 'full_name', 'avatar');
        ...
        if (in_array(HomeSection::$discount_classes, $selectedSectionsName)) {
            ...
            */
            ...
            $hasDiscountWebinars = Webinar::where('status', Webinar::$active)->where('private', false)->where('category_id', 525)
            ->orderBy('updated_at', 'desc')
            ->with([
                'teacher' => function ($qu) {
                    $qu->select('id', 'full_name', 'avatar');
